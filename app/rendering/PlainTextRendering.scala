...
class CandidateListSection(
  ...
  def this(c: Candidate) =
    this(c.name, c.candidate, c.description, c.default.getOrElse("Coming soon!"), c.websiteUrl)
}
...
trait PlainTextRendering extends WordWrapping {
  ...
  def header: String =
    s"$prettyName ($defaultVersion)".padTo(ConsoleWidth - websiteUrl.length, ' ') + websiteUrl
