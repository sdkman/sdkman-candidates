...
class RandomOrderingSpec
  ...
  "regex string ordering" should {
    ...
    val candidateVersions = Source
      .fromFile(s"test/resources/candidates.csv")
      .getLines
      .toList
    ...
    "accurately order actual versions" in new VersionItemOrdering {
      ...
      candidateVersions.foreach { case (candidate, versions) =>
        ...
        check {
          Prop.forAll(Gen.pick(fullExpected.length - 1, fullExpected.toSet).map(_.toList)) {
            xs: List[VersionItem] =>
              val partialActual = xs.ascendingOrder

              logger.info(
                s"$candidate shuffled: ${xs.map(_.version).mkString(",")} ===> ordered: ${partialActual.map(_.version).mkString(",")}"
              )
